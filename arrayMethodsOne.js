...
// CODE HERE
const myStrongest = monstersInYourPocket.filter(ele => ele.CP > 200)
...
// CODE HERE
const taxedPrice = orders.map((ele) => +(ele.price * (ele.tax + 1)).toFixed(2))
...
const bobsTotal = purchases.filter(ele => ele.owner.includes(`Bob`)).reduce((acc, curr) => acc + curr.price,0)
